...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:		
      return mineCard(choice1, choice2, state, handPos, currentPlayer);
    ...
    case tribute:
      return tributeCard(state, currentPlayer);
...
{
      state->numBuys = state->numBuys + 2;//Increase buys by 1!  bugAdded
	...
	while(card_not_discarded){
	  if (state->hand[currentPlayer][p] == estate){//Found an estate card!
	    ...
	    if (supplyCount(estate, state) > 0){
	      ...
	      state->supplyCount[estate] = state->supplyCount[estate] - 2;//Decrement estates  bugAdded
	      if (supplyCount(estate, state) == 0){
		isGameOver(state);
	      }
	    }
...
//	  if (i != currentPlayer)                                bugAdded
      ...
      //discard played card from hand
      discardCard(handPos+1, currentPlayer, state, 0);		// bugAdded	
...
/*  Tribute  no comments in the dominion.h file for this card */
int tributeCard(struct gameState *state, int currentPlayer)
{
      int i;
      int tributeRevealedCards[2] = {-1, -1};
      int nextPlayer = currentPlayer + 4;           //  bugAdded
	...
	if (tributeRevealedCards[i] == copper || tributeRevealedCards[i] == silver || tributeRevealedCards[i] == gold){//Treasure cards
	  ...
	  state->numActions = state->numActions - 2;    // bugAdded
	}
      }
      return 0;
}
...
int mineCard(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer)
{
      ...
      j = state->hand[currentPlayer][choice2];  //store card we will trash     bugAdded
      ...
      if ( (getCost(state->hand[currentPlayer][choice1]) + 3) > getCost(choice2) )
	{
      ...
      gainCard(choice1, state, 2, currentPlayer);          //   bugAdded
